...
const PaginationNav = ({ totalPages, currentPage, handlePagination }) => {
  ...
  const boxNumberStyle = "flex items-center justify-center w-10 h-10 border";
  ...
  const PaginationButton = ({ number, isCurrent, onClick, disabled }) => (
    <button
      ...
      } ${boxNumberStyle} ${disabled ? "cursor-not-allowed" : ""}`}
    >
      {number}
    </button>
  );
  ...
  const ArrowButton = ({ direction, disabled, onClick }) => (
    <button onClick={onClick} disabled={disabled} className={boxNumberStyle}>
